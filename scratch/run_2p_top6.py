...
AGENTS = {
    ...
    "best-orbit-wars-notebook": os.path.join(_BASE, "extracted_agents/best-orbit-wars-notebook_main.py"),
...
def run_match_worker(args):
    ...
    try:
        for idx, path in enumerate(agent_paths):
            ...
            module_name = f"top6_worker_{idx}_{os.getpid()}_{id(path)}_{random.randint(0, 1000000)}"
        ...
        for idx, s in enumerate(final_step):
            results.append({"name": player_names[idx], "reward": s.reward})
        return {"success": True, "results": results, "seed": seed}
    except Exception as e:
        return {"success": False, "error": f"Match crash: {str(e)}"}
    finally:
        loaded_agents = None
...
def run_tournament():
    ...
    print(f"총 게임 수: {len(tasks)}", flush=True)
    ...
    with ProcessPoolExecutor(max_workers=14) as executor:
        ...
        for future in as_completed(futures):
            ...
            if done % 50 == 0:
                print(f"  {done}/{len(tasks)} 완료", flush=True)
    ...
    print(f"\n{'순위':<5}{'봇':<45}{'승':<8}{'게임':<8}{'승률'}")
    print("-" * 70)
    for rank, (name, w, p, rate) in enumerate(leaderboard, 1):
        print(f"{rank:<5}{name:<45}{w:<8.1f}{p:<8}{rate:.2f}%")
